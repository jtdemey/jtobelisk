...
const createSchema = (database) => {
    ...
    database
        ...
        .run(TEST_EMAIL);
    database
        .prepare("DELETE FROM verified_list WHERE email = ?")
        .run(TEST_EMAIL);
};
...
export const createEmailList = (
    ...
    router.route(subscribeEndpoint).post((req, res) => {
        ...
        // Disaster safeguard
        const countRecords = db.prepare("SELECT email FROM unverified_list");
    ...
    router.route(verifyEndpoint).get((req, res) => {
        ...
        const matchingVerificationCode = getVerificationCode.run(
            req.query.email,
        );
