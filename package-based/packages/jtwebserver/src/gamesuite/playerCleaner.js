...
export const pingPlayers = (gameSuite) => {
    ...
    gameSuite.playerList.forEach((player) => {
        ...
        const pinged = pingedPlayers.filter(
            (x) => x.socketId === player.socketId,
        );
        if (pinged.length === 0) {
            pingedPlayers.push({
        ...
        if (pinged[0].tries > 2) {
            ...
            logger.info(
                `[GS] Player ${player.socketId} removed due to inactivity.`,
            );
            return;
        }
        logger.info(
            `[GS] No pong from ${player.socketId} (Attempts: ${pinged[0].tries + 1}).`,
        );
        pinged[0].tries += 1;
    });
};
