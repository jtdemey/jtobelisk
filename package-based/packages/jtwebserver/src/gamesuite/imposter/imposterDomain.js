...
const createImposterDomain = (gameSuite) => {
  ...
  domain.castVote = (msg) => {
    ...
    if (!vote) {
      gameSuite.logError(
        `Cast vote: Unable to find vote ${msg.voteId} in ${msg.gameId}`
      );
    }
    if (
      vote.callerId === msg.socketId ||
      vote.voters.some((v) => v === msg.socketId)
    ) {
      return;
    }
    if (msg.isYay) {
  ...
  domain.handleAccusePlayer = (msg) => {
    const currGame = gameSuite.getGame(msg.gameId);
    if (
      msg.accuserId === msg.accusedId ||
      currGame.votes.some((v) => v.callerId === msg.accuserId)
    ) {
      return;
    }
    const callerName = currGame.players.filter(
      (p) => p.socketId === msg.accuserId
    )[0].name;
    if (!callerName) {
      gameSuite.logError(
        `Accuse player: Unable to find player name for accuser ${msg.accuserId}`
      );
    }
    const accusedName = currGame.players.filter(
      (p) => p.socketId === msg.accusedId
    )[0].name;
    if (!accusedName) {
      gameSuite.logError(
        `Accuse player: Unable to find player name for accused ${msg.accuserId}`
      );
    }
    const thresh =
      currGame.players.length < 3 ? 1 : currGame.players.length - 2;
    const accusation = gameSuite.makeVote(
      VOTE_TYPES.ACCUSATION,
      msg.accuserId,
      callerName,
      thresh,
      {
    ...
    gameSuite.emitToGame(
      currGame.gameId,
      gameSuite.makeCommand(SOCKET_COMMANDS.REFRESH_VOTES, {
        votes: newVotes,
      })
    );
    gameSuite.logInfo(
      `${msg.gameId}: ${msg.accuserId} accuses ${msg.accusedId}`
    );
  };
  ...
  domain.handleLobbyReturnVote = (msg) => {
    ...
    const callerName = currGame.players.filter(
      (p) => p.socketId === msg.socketId
    )[0].name;
    if (!callerName) {
      gameSuite.logError(
        `Return to lobby: Unable to find player name for ${msg.socketId}`
      );
    }
    const thresh =
      currGame.players.length - 1 < 1 ? 1 : currGame.players.length - 1;
    const vote = gameSuite.makeVote(
      VOTE_TYPES.LOBBY,
      msg.socketId,
      callerName,
      thresh
    ...
    gameSuite.logInfo(
      `${msg.gameId}: ${msg.socketId} votes to return to the lobby`
    );
  };
  ...
  domain.identifyScenario = (msg) => {
    ...
    if (imposter.length < 1) {
      gameSuite.logError(
        `Identify scenario: unable to find imposter with ID ${msg.imposterId}`
      );
    }
    if (game.scenario.toUpperCase() === msg.scenario.toUpperCase()) {
      ...
      gameSuite.logInfo(
        `Imposter for ${msg.gameId} guessed the wrong scenario`
      );
    }
  };
